...
class BaseMeta(Module):
    ...
    def __init__(self, nfeat, hidden_sizes, nclass, nnodes, dropout, train_iters, attack_features, lambda_, device, with_bias=False, lr=0.01, with_relu=False):
        ...
        self.hitsgnn = HITSGNN_ADAPTIVE(nfeat=nfeat,
                       nhid=hidden_sizes[0],
                       nclass=nclass,
                       dropout=0.5,
                       with_relu=False)
        ...
        self.surrogate_optimizer = optim.Adam(self.hitsgnn.parameters(), lr=lr, weight_decay=5e-4)
...
class Metattack(BaseMeta):
    ...
    def get_meta_grad(self, features, adj_norm, idx_train, idx_unlabeled, labels, labels_self_training):
        ...
        print(f'HITSGNN loss on unlabled data: {loss_test_val.item()}')
        print(f'HITSGNN acc on unlabled data: {utils.accuracy(output[idx_unlabeled], labels[idx_unlabeled]).item()}')
