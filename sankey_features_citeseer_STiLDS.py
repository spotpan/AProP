...
def feature_test(adj, modified_adj, features, labels, idx_train, idx_test, flag):

    cst=370 #num of feature each sect 
    runs=10 #num of sect


    for i in range(runs):
        innerfeatures=features[:,cst*i:cst*(i+1)]
        print("features", innerfeatures.size())
        res1=test(adj, innerfeatures, labels, idx_train, idx_test)
        res2=test(modified_adj, innerfeatures, labels, idx_train, idx_test)
        res=res1-res2
        #clean.acc - poisoning.acc
        #the larger one is the stronger one

        diff_acc.append( (f"n{flag+1}", f"segment{i+1}", res) )

        print("current flag", flag+1 ,"acc:", res, "at sect", i+1)
...
def main():
    ...
    idx_test=[idx_test1, idx_test2, idx_test3, idx_test4, idx_test5, idx_test6]
    ...
    print('=== feature testing between clean graph and poisoning graph ===')
    ...
    for list in idx_test:

        
        idx_test=np.array(list)
        ...
        print("len of idx_test", idx_test.size)

        print("feature size",features.size())
        
        feature_test(adj, modified_adj, features, labels, idx_train, idx_test, flag)
